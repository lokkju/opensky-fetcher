...
class TestFlightDatabase:
    ...
    def test_has_data_empty_database(self, temp_db_path):
        ...
        result = db.has_data("KMCO", date(2024, 1, 1), "departure")
    ...
    def test_has_data_after_insert(self, temp_db_path):
        ...
        # Insert raw response
        db.insert_raw_response("KMCO", date(2024, 1, 1), "departure", "[]")
        db.commit()
    ...
    def test_insert_raw_response(self, temp_db_path):
        ...
        db.insert_raw_response("KMCO", date(2024, 1, 1), "departure", test_data)
        db.commit()

        # Verify insertion
        result = db.conn.execute(
            "SELECT raw_json FROM raw_responses WHERE airport = ? AND date = ? AND flight_type = ?",
            ["KMCO", date(2024, 1, 1), "departure"],
        ).fetchone()
    ...
    def test_insert_raw_response_replace(self, temp_db_path):
        ...
        db.insert_raw_response("KMCO", date(2024, 1, 1), "departure", new_data)
        db.commit()
    ...
    def test_insert_flights(self, temp_db_path):
        ...
        db.insert_flights("KMCO", date(2024, 1, 1), "departure", flights)
        db.commit()
    ...
    def test_insert_flights_replaces_existing(self, temp_db_path):
        ...
        db.insert_flights("KMCO", date(2024, 1, 1), "departure", flights1)
        db.commit()
        ...
        db.insert_flights("KMCO", date(2024, 1, 1), "departure", flights2)
        db.commit()
    ...
    def test_context_manager(self, temp_db_path):
        ...
        result = db2.has_data("KMCO", date(2024, 1, 1), "departure")
        assert result is True
        db2.close()
